    ...
    <main className="landingPage">
      ...
      <section className="landingSection pricingSection" id="pricing">
        ...
        {plan ? <div className="pricingGrid">
          <article><span>{locale === "en" ? "Free" : plan.metadata.freeTier.name.zh}</span><h3>${(plan.metadata.freeTier.amountCents / 100).toFixed(0)}</h3><p>{locale === "en" ? "Learn the complete workflow with your own compute and storage." : plan.metadata.freeTier.summary.zh}</p><strong>{locale === "en" ? "1 active project" : `${plan.metadata.quotas.projects.free}个活跃项目`}</strong><Link className="landingSecondary" href="/workbench/project">{t("开始使用")}</Link></article>
          <article className="featuredPrice"><span>{locale === "en" ? "For active projects" : plan.metadata.customerDisplay.zh.offerLabel}</span><h3>${(plan.amountCents / 100).toFixed(0)} <small>{locale === "en" ? "/ year" : plan.metadata.customerDisplay.zh.billingSuffix}</small></h3><p>{locale === "en" ? "Manage more active training projects with the same data and compute boundaries." : plan.metadata.customerDisplay.zh.summary}</p><strong>{plan.metadata.quotas.projects.paid}{t("个活跃项目")} · {locale === "en" ? "API connections" : "API 连接"}</strong><Link className="landingPrimary" href="/api/billing/checkout">{t("升级 Pro")} <span aria-hidden="true">→</span></Link></article>
