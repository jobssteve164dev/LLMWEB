...
import { TrainingParameterEditor, profileForTrainingParameters, profileTrainingParameters, trainingEstimate, trainingParametersFromForm, type TrainingParameters, type TrainingPlanInput, type TrainingPlanPreview } from "./training-parameter-editor";
...
export function Workbench() {
  ...
  const [datasetEditRequested, setDatasetEditRequested] = useState(false);
      ...
      <div className="workspaceLayout">
        ...
        <section className="workAreaScroll">
          <div className="workArea">
            ...
            {activeStep === "data" ? <DataStep project={project} runner={runner} datasets={runnerDatasets} dataset={dataset} busy={busy} perform={perform} moveTo={moveTo} onSelect={setSelectedDatasetId} editRequested={datasetEditRequested} onEditHandled={() => setDatasetEditRequested(false)} /> : null}
            {activeStep === "train" ? <TrainStep key={runner?.id ?? "no-runner"} project={project} runner={runner} datasets={runnerDatasets} dataset={dataset} busy={busy} perform={perform} moveTo={moveTo} onSelect={setSelectedDatasetId} onEdit={() => { setDatasetEditRequested(true); moveTo("data"); }} /> : null}
...
function DatasetVersionPicker({ datasets, dataset, onSelect }: { datasets: Dataset[]; dataset: Dataset; onSelect: (datasetId: string) => void }) {
  const { locale } = useLanguage(); const english = locale === "en";
  const readyDatasets = datasets.filter((item) => item.status === "ready");
  return <section className="datasetVersionCard" aria-labelledby="dataset-version-title"><div className="cardHeading"><div><span>{english ? "Training data" : "训练数据"}</span><h2 id="dataset-version-title">{english ? "Choose the version used for this run" : "选择本次训练使用的数据版本"}</h2></div><small>{readyDatasets.length} {english ? "ready" : "个可用版本"}</small></div><div className="datasetVersions" aria-label={english ? "Data versions" : "数据版本列表"}>{readyDatasets.map((item) => <button className={item.id === dataset.id ? "selected" : ""} type="button" key={item.id} onClick={() => onSelect(item.id)}><span><strong>{item.name}</strong><small>{item.statistics?.valid_rows ?? item.statistics?.characters ?? 0} {item.source_type === "starter" ? (english ? "characters" : "个字符") : (english ? "usable samples" : "条可用样本")}</small></span><em>{item.id === dataset.id ? (english ? "In use" : "当前使用") : (english ? "Choose" : "选择")}</em></button>)}</div></section>;
}

function DatasetGuide() {
  const { locale } = useLanguage(); const english = locale === "en";
  return <details className="datasetGuide" open><summary>{english ? "Sample format for your own data" : "自己的数据应该怎样准备"}</summary><div><p>{english ? "Use UTF-8. An archive must contain exactly one JSONL, JSON, or CSV file and expand to no more than 256 MB." : "文件使用 UTF-8 编码。压缩包内只能有一个 JSONL、JSON 或 CSV 文件，解压后不超过 256 MB。"}</p><pre>{'{"instruction":"用户的问题","input":"可选的补充信息","output":"期望模型给出的回答"}'}</pre><ul><li>{english ? "instruction and output are required; input may be empty." : "instruction 和 output 必填，input 可以为空字符串。"}</li><li>{english ? "Use one object per line for JSONL; JSON uses an array; CSV uses a header row." : "JSONL 每行一条对象；JSON 使用对象数组；CSV 第一行是字段名。"}</li><li>{english ? "Prepare at least 30 high-quality, non-duplicate samples; 3 is only the technical minimum." : "建议至少准备 30 条高质量、不重复样本；3 条只是技术最低值。"}</li></ul></div></details>;
}

function DataStep({ project, runner, datasets, dataset, busy, perform, moveTo, onSelect, editRequested, onEditHandled }: { project: Project | null; runner: Runner | null; datasets: Dataset[]; dataset: Dataset | null; busy: boolean; perform: Perform; moveTo: (step: Step) => void; onSelect: (datasetId: string) => void; editRequested: boolean; onEditHandled: () => void }) {
  const { locale } = useLanguage(); const english = locale === "en";
  const [sourceType, setSourceType] = useState<Dataset["source_type"]>(() => editRequested && dataset?.source_type !== "starter" ? dataset?.source_type ?? "local" : "local");
  ...
  if (dataset?.status === "ready" && dataset.statistics && !editing) return <><SectionIntro eyebrow={english ? "Data ready" : "数据已就绪"} title={dataset.name} description={english ? "Choose any checked version for training, or edit its settings to create a new immutable version." : "你可以选择任一已检查版本用于训练，也可以修改设置并建立一个新的不可变版本。"} />
    <DatasetVersionPicker datasets={datasets} dataset={dataset} onSelect={onSelect} />
    <DatasetReport dataset={dataset} /><div className="dataReadyGuide"><DatasetGuide /></div><div className="formActions"><button className="secondaryButton" type="button" onClick={() => { setSourceType(dataset.source_type === "starter" ? "local" : dataset.source_type); setEditing(true); }}>{english ? "Edit and create new version" : "编辑并建立新版本"}</button><button className="primaryButton" type="button" onClick={() => moveTo("train")}>{english ? "Train with this data" : "用这份数据开始训练"}</button></div></>;
  if (isCPU && !editing && dataset?.source_type !== "local" && dataset?.source_type !== "huggingface" && dataset?.source_type !== "modelscope" && dataset?.source_type !== "s3") return <><SectionIntro eyebrow={english ? "Step three" : "第三步"} title={english ? "Complete your first run with an example." : "先用示例完成第一次训练。"} description={english ? "LLMWEB prepares Shakespeare text so a small model can learn next-character prediction. After this run, you will understand every training stage." : "LLMWEB 会准备一份莎士比亚文本，让小模型学习如何逐字续写。完成这一遍后，你会看懂训练的每个阶段。"} />
  ...
  return <><SectionIntro eyebrow={editing ? (english ? "New data version" : "建立新数据版本") : (english ? "Step three" : "第三步")} title={english ? "Choose your data, check it, then train." : "选择自己的数据，先检查再训练。"} description={english ? "Files are read directly on the selected computer. They are never uploaded to LLMWEB." : "文件由所选训练电脑直接读取，不会上传到 LLMWEB。"} />
    ...
    <form className="formCard" onSubmit={(event) => {
      ...
      <div className="formActions">{editing ? <button className="secondaryButton" type="button" onClick={() => { setEditing(false); onEditHandled(); }}>{english ? "Cancel" : "取消"}</button> : null}<button className="primaryButton" disabled={busy} type="submit">{busy ? (english ? "Submitting…" : "正在提交…") : (english ? "Check and create data version" : "检查并建立数据版本")}</button></div>
    </form></>;
}
...
function TrainingDatasetSetup({ datasets, dataset, onSelect, onEdit }: { datasets: Dataset[]; dataset: Dataset; onSelect: (datasetId: string) => void; onEdit: () => void }) {
  const { locale } = useLanguage(); const english = locale === "en";
  return <div className="trainingDatasetSetup"><DatasetVersionPicker datasets={datasets} dataset={dataset} onSelect={onSelect} /><DatasetGuide /><div className="formActions"><button className="secondaryButton" type="button" onClick={onEdit}>{english ? "Edit and create new version" : "编辑并建立新版本"}</button></div></div>;
}

function TrainStep({ project, runner, datasets, dataset, busy, perform, moveTo, onSelect, onEdit }: { project: Project | null; runner: Runner | null; datasets: Dataset[]; dataset: Dataset | null; busy: boolean; perform: Perform; moveTo: (step: Step) => void; onSelect: (datasetId: string) => void; onEdit: () => void }) {
  const { locale } = useLanguage(); const english = locale === "en";
  ...
  const [selectedMethod, setSelectedMethod] = useState<"lora" | "qlora">(runner?.capabilities.backend === "native_mps" ? "lora" : "qlora");
  const [trainingParameters, setTrainingParameters] = useState<TrainingParameters>(() => runner?.capabilities.backend === "docker_cpu"
    ? { epochs: 3, learning_rate: 0.001, max_length: 64, batch_size: 12, gradient_accumulation: 1 }
    : profileTrainingParameters("balanced"));
  const previewPlan = useCallback((input: TrainingPlanInput) => api<TrainingPlanPreview>("training-plans/preview", { method: "POST", body: JSON.stringify(input) }), []);
  ...
  if (isCPU) {
    ...
    const profileLabel = starterProfiles.find(([value]) => value === profile)?.[1] ?? (english ? "Custom settings" : "自定义设置");
    return <><SectionIntro eyebrow={english ? "Step four" : "第四步"} title={english ? "Choose how long to wait, then start." : "选择愿意等待多久，然后开始。"} description={english ? "The model and settings are already matched to this computer. Choose a duration and the system completes the remaining steps in order." : "模型和训练设置已经按这台电脑匹配好。你只需要选择时长，剩下的步骤由系统依次完成。"} />
      <TrainingDatasetSetup datasets={datasets} dataset={dataset} onSelect={onSelect} onEdit={onEdit} />
      ...
      <form className="formCard starterTraining" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); const parameters = trainingParametersFromForm(form); const planInput: TrainingPlanInput = { runner_id: runner.id, model_id: "karpathy/nanoGPT", method: "starter", ...parameters }; void perform(async () => { await previewPlan(planInput); return api("experiments", { method: "POST", body: JSON.stringify({ project_id: project.id, dataset_id: dataset.id, name: form.get("name"), ...planInput, export_formats: ["model"], evaluation_preview_allowed: true, output_destination: "local", license_confirmed: form.get("license_confirmed") === "on" }) }); }, english ? "Training started. You can close the web app; the computer will continue." : "训练已开始。网页可以关闭，电脑会继续完成。") }}>
        ...
        <fieldset className="choiceField"><legend>{english ? "I am willing to wait" : "我愿意等待"}</legend><div className="profileChoices starterProfiles">{starterProfiles.map(([value, name, time, description]) => <label key={value} className={profile === value ? "selected" : ""}><input type="radio" name="profile" value={value} checked={profile === value} onChange={() => { setProfile(value); setTrainingParameters({ epochs: value === "fast" ? 1 : value === "thorough" ? 5 : 3, learning_rate: 0.001, max_length: 64, batch_size: 12, gradient_accumulation: 1 }); }} /><strong>{name}</strong><small>{time}</small><em>{description}</em></label>)}</div></fieldset>
        <TrainingParameterEditor locale={locale} runnerId={runner.id} modelId="karpathy/nanoGPT" method="starter" values={trainingParameters} editableKeys={[]} onParametersChange={setTrainingParameters} previewPlan={previewPlan} />
        ...
        <label className="consentRow"><input type="checkbox" name="license_confirmed" required /><span>{dataset.source_type === "starter" ? (english ? "I agree to use the included public practice material for this starter run" : "我同意使用内置公开练习材料完成这次入门训练") : (english ? "I confirm that I have the right to train with this data" : "我确认有权使用这份数据进行训练")}</span></label>
  ...
  const activeProfile = profileForTrainingParameters(trainingParameters);
  const estimate = trainingEstimate(selectedModel, trainingParameters, dataset.statistics?.valid_rows ?? 0, locale);
  return <><SectionIntro eyebrow={english ? "Step four" : "第四步"} title={english ? "Choose a model and training intensity." : "选择模型和训练强度。"} description={english ? "The system measures the base model first, then trains and retests on the same test set." : "系统会先测基础模型，再训练并用同一测试集复测。"} />
    <TrainingDatasetSetup datasets={datasets} dataset={dataset} onSelect={onSelect} onEdit={onEdit} />
    <form className="formCard" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); const parameters = trainingParametersFromForm(form); const formats = ["adapter", ...(form.get("huggingface") ? ["huggingface"] : []), ...(form.get("gguf") ? ["gguf"] : [])]; const planInput: TrainingPlanInput = { runner_id: runner.id, model_id: selectedModel, method: selectedMethod, ...parameters }; void perform(async () => { await previewPlan(planInput); return api("experiments", { method: "POST", body: JSON.stringify({ project_id: project.id, dataset_id: dataset.id, name: form.get("name"), ...planInput, export_formats: formats, evaluation_preview_allowed: form.get("evaluation_preview") === "on", output_destination: outputDestination, output_s3_uri: form.get("s3_uri") || null, output_s3_endpoint: form.get("s3_endpoint") || null, license_confirmed: form.get("license_confirmed") === "on" }) }); }, english ? "Training workflow started. The base-model baseline runs first." : "训练流程已启动，会先建立基础模型基线。 "); }}>
...
function ModelChat({ experiment, jobs, runner, busy, perform }: { experiment: Experiment; jobs: Job[]; runner: Runner | null; busy: boolean; perform: Perform }) {
  ...
  const supportsChat = runner?.capabilities.supported_tasks?.includes("chat") === true;
  ...
  return <section className="modelChat"><div className="cardHeading"><div><span>{english ? "Try the trained model" : "直接试用训练后的模型"}</span><h2>{experiment.training.method === "starter" ? (english ? "Give it a beginning and see how it continues" : "给出一段开头，看看模型怎样续写") : (english ? "Chat with this model" : "和这份模型对话")}</h2></div><strong className={runner?.status === "online" ? "evidenceBadge" : "evidenceBadge warning"}>{runner?.status === "online" ? (english ? "Computer online" : "电脑在线") : (english ? "Computer offline" : "电脑离线")}</strong></div>
    ...
    <form className="chatComposer" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); const prompt = String(form.get("prompt") ?? "").trim(); if (!prompt) return; void perform(() => api(`experiments/${experiment.id}/chat`, { method: "POST", body: JSON.stringify({ prompt }) }), english ? "Message sent to your training computer." : "消息已发送到训练电脑。", false).then((sent) => { if (sent) event.currentTarget.reset(); }); }}><label><span className="srOnly">{english ? "Message" : "测试内容"}</span><textarea name="prompt" required maxLength={4000} rows={3} placeholder={experiment.training.method === "starter" ? (english ? "For example: ROMEO:" : "例如：ROMEO:") : (english ? "Type a question for the model…" : "输入想让模型回答的问题…")} /></label><button className="primaryButton" disabled={busy || pending || runner?.status !== "online" || !supportsChat} type="submit">{pending ? (english ? "Generating…" : "正在生成…") : (english ? "Send" : "发送")}</button></form>
    {runner?.status !== "online" ? <small className="chatHint">{english ? "Bring the computer used for this training online to test the local model." : "请先让执行本次训练的电脑上线，才能测试保存在本地的模型。"}</small> : !supportsChat ? <small className="chatHint">{english ? "Upgrade this computer’s training environment to enable model chat." : "升级这台电脑的训练环境后，即可使用模型对话。"}</small> : null}
  </section>;
}
