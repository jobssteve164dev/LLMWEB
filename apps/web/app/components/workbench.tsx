...
function MachineLive({ runner }: { runner: Runner }) {
  ...
  if (sharedMemory) return <div className="gpuLive"><div><span>加速后端</span><strong>Metal / MPS</strong></div><div><span>统一内存</span><strong>{(total / 1024).toFixed(0)} GB</strong></div><div><span>运行设备</span><strong>Apple Silicon GPU</strong></div></div>;
