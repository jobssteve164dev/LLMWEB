...
export type TrainingPlanPreview = {
  ...
  resolved: TrainingParameters & { method: "lora" | "qlora" };
...
export function TrainingParameterEditor({ locale, runnerId, modelId, method, values, onParametersChange, previewPlan }: Props) {
  ...
  const labels: Record<keyof TrainingParameters, [string, string, string, string]> = {
    ...
    max_length: ["上下文长度", "Context length", "每条样本最多使用的 token 数", "Maximum tokens used from each sample"],
  ...
  return h("details", { className: "trainingParameterEditor" },
    h("summary", null,
      h("span", null, english ? "Adjust training parameters" : "调整训练参数"),
      h("small", null, custom ? (english ? "Custom settings are active" : "正在使用自定义设置") : (english ? "Recommended settings are active" : "正在使用推荐设置")),
    ),
    h("div", { className: "trainingParameterGrid" }, parameterKeys.map((key) => {
      ...
      return h("label", { key },
        ...
        h("input", {
          ...
          value: values[key],
          onChange: (event: { currentTarget: { value: string } }) => onParametersChange({ ...values, [key]: Number(event.currentTarget.value) }),
        }),
