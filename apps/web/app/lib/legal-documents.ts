...
async function fetchLegal<T>(path: string, key: "document" | "supplement"): Promise<T> {
  const response = await fetch(`${legalBaseUrl}${path}`, { cache: "no-store", headers: { accept: "application/json" } });
  if (!response.ok) throw new Error(`法律文件暂时无法读取（${response.status}）。`);
  const payload = await response.json() as { success: boolean; document?: T; supplement?: T; error?: { message?: string } };
  const value = payload[key];
  if (!payload.success || !value) throw new Error(payload.error?.message || "法律文件暂时无法读取。");
  return value;
}

export function getSharedLegalDocument(type: LegalDocumentType) {
  const query = new URLSearchParams({ product: productId, type, locale: "zh-CN" });
  return fetchLegal<SharedLegalDocument>(`/api/legal/document?${query}`, "document");
}

export function getProductLegalSupplement() {
  const query = new URLSearchParams({ product: productId, locale: "zh-CN" });
  return fetchLegal<ProductLegalSupplement>(`/api/legal/product-supplement?${query}`, "supplement");
}
