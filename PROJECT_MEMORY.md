...
- 训练参数区在“开始训练”页面默认展开。GPU 与 Apple Silicon 训练可调整 epochs、learning rate、max length、batch size 和 gradient accumulation；CPU 入门训练用三档强度选择 200/500/1000 个真实训练步，参数区展示引擎真实采用的学习率、64 字符窗口、批量和梯度累积并保持只读。预检与实验创建复用 Runner 完整能力校验和同一解析器，实验保存计划版本、实际训练步数、有效批次及与引擎一致的算子快照，Runner 任务仍只接收既有 1.0 协议字段。
