...
训练参数区直接显示在“开始训练”页面。GPU 或 Apple Silicon 训练可以保留“快速、均衡、充分”推荐设置，也可以调整训练轮次、学习率、上下文长度、单次批量和梯度累积；普通 CPU 入门训练通过三档训练强度选择真实训练步数，参数区展示系统为当前电脑匹配的实际值。控制面使用与正式创建训练相同的解析器预检设置，并显示实际训练步数、有效批次与固定执行策略。
...
当前仓库已覆盖项目创建、一次性配对、数据检查、训练参数预检、基础模型评测、LoRA/QLoRA 训练、训练版本选择、固定测试集复测、暂停/继续/取消、断线事件补传及产物导出。没有受支持 GPU 的开发机可以验证网页与控制流程，但不能替代 Linux NVIDIA 或 Apple Silicon 上的真实训练验收。
