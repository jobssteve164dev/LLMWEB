...
在支持的 GPU 或 Apple Silicon 训练方案中，用户可以保留“快速、均衡、充分”推荐设置，也可以在同一训练页面调整训练轮次、学习率、上下文长度、单次批量和梯度累积。控制面会使用与正式创建训练相同的解析器预检这组设置，显示有效批次与当前固定执行策略；普通 CPU 入门训练仍由系统自动匹配参数。
